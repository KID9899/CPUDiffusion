...
public:
    ...
    Vae():
           encoder({
                   ...
                   std::make_shared<Linear>(28*28, 256),
                   std::make_shared<ReLU>(),
                   std::make_shared<Linear>(256, 128),
                   std::make_shared<ReLU>()
           }),
           mu(128, latent_dim),
           logvar(128, latent_dim),
           decoder({
                   std::make_shared<Linear>(latent_dim, 128),
                   std::make_shared<ReLU>(),
                   std::make_shared<Linear>(128, 256),
                   std::make_shared<ReLU>(),
                   std::make_shared<Linear>(256, 28*28),
...
int main() {
    ...
    SafeTensorsFile vae_file("samples/simple_vae_mnist.safetensors");
