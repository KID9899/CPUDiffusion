...
int main() {
    ...
    auto generate_text = [](const std::string &prompt) {
        ...
        SafeTensorsFile st_file("samples/simple_transformer.safetensors");
        Tokenizer tokenizer("samples/simple_transformer_vocab.txt");
        tokenizer.load_merges("samples/simple_transformer_merges.txt");
...
// Визуализация графа
// dot -Gnslimit=2 -Gsplines=false -v myfile.gv -Tsvg -o render.svg *.dot
